...
const ManajemenAdmin = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        // --- TEKNIK KONEKSI SEMENTARA ---
        // 1. Buat koneksi sementara ke Firebase
        const secondaryAppConfig = auth.app.options;
        const secondaryApp = initializeApp(secondaryAppConfig, `secondary-${Date.now()}`);
        const secondaryAuth = getAuth(secondaryApp);

        try {
            // 2. Buat user baru menggunakan koneksi sementara
            const userCredential = await createUserWithEmailAndPassword(secondaryAuth, formData.email, formData.password);
            const newUser = userCredential.user;

            // 3. Simpan data user ke Firestore menggunakan koneksi utama
            ...
            // 4. Hapus koneksi sementara (opsional, tapi praktik yang baik)
    ...
    return (
        <div className="bg-white p-6 rounded-xl shadow-md">
            ...
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-bold">Daftar Admin Desa</h2>
                    ...
                    <tbody>
                        {adminList.map((admin) => (
                            <tr key={admin.id} className="bg-white border-b hover:bg-gray-50">
                                <td className="px-6 py-4 font-medium">{admin.nama}</td>
                                <td className="px-6 py-4">{admin.email}</td>
                                <td className="px-6 py-4">{admin.desa}</td>
                                <td className="px-6 py-4 flex space-x-3">
                                    <button onClick={() => handleResetPassword(admin.email)} className="text-yellow-600 hover:text-yellow-800" title="Reset Password"><FiKey /></button>
                                    <button onClick={() => handleDelete(admin.id, admin.nama)} className="text-red-600 hover:text-red-800" title="Hapus"><FiTrash2 /></button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title="Tambah Admin Desa Baru">
                <form onSubmit={handleFormSubmit} className="space-y-4">
                    <div>
                        ...
                        <input type="text" name="nama" value={formData.nama || ''} onChange={handleFormChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2" required />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Email</label>
                        <input type="email" name="email" value={formData.email || ''} onChange={handleFormChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2" required />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Password</label>
                        <div className="relative">
                            <input type={showPassword ? "text" : "password"} name="password" value={formData.password || ''} onChange={handleFormChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2" required />
                    ...
                    <div>
                        ...
                        <select name="desa" value={formData.desa || ''} onChange={handleFormChange} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm p-2" required>
                            {DESA_LIST.map(desa => <option key={desa} value={desa}>{desa}</option>)}
                        </select>
                    </div>
                    <div className="flex justify-end pt-4 border-t mt-6">
                        <button type="button" onClick={handleCloseModal} className="px-4 py-2 bg-gray-200 rounded-md mr-2" disabled={isSubmitting}>Batal</button>
